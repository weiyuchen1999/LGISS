function [Subset] = LazyHVSelection(PopObj)
    a = size(PopObj, 1);
    M = size(PopObj, 2);
    selectedPop = [];
    ...
    for n = 1 : a
       ...
       heap.hvc(n) = HVC(currentPop, max(PopObj,[],1)*1.1, size(currentPop, 1));
    ...
    while size(selectedPop, 1) < 100
        while true
            last_index = heap.index(1);
            top = heap.index(1);
            cuurentPop = [selectedPop; PopObj(top, :)];
            heap.hvc(1) = HVC(cuurentPop, max(PopObj,[],1)*1.1, size(cuurentPop, 1));
            heap = heap_down_sink(1, heap);
            
			%% If the re-evaluated HVC is still the largest
